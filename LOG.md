...
- [ ] 0% -- 更新设置时，同步更新 `settings.json` 文件。
- [ ] 0% -- 编辑 "激活热键" 时，禁用当前热键，防止误触关闭剪贴板。
- [ ] 0% -- 设计数据库 API：查询、保存、删除、更新。
- [ ] 0% -- 完善监听剪贴板 SDK：监听剪贴板变化、获取当前剪贴板类型、将文件路径写入剪贴板。
- [ ] 0% -- 如何检查更新，以及更新到最新版本。
...
- 关于 `settings` 的更新，考虑到在任务栏里也能更新一些配置，以及系统主题更新后，需要通知剪贴板更新。由
