...
- 昨天看书《微前端的设计与实现》里提到了一种架构设计：前端层、API 层、数据库。然后联想到最近在预研这个剪贴板功能实现的时候，大多数都是通过主进程 `ipcMain` 和渲染进程 `ipcRender` 间进行通信，以及需要用到 `IndexedDB` 去存储剪贴板的历史记录，刚好和这种设计不谋而合。前端层对应的就是页面，数据库对应 `IndexedDB`，而 API 层就可以作为主进程和渲染进程间通信的抽象，以及前端层需要操作数据库的抽象。所以需要重点设计这个 API 层，下列操作都通过 API 层去实现：
